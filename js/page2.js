...
Page2.prototype = {
    ...
    render: function(ctx) {
        ...
        d3.xml('/svg/page2.svg', function(xml) {
            ...
            replace_yesno(page2.select('#country-compact'), 87, -340, ind['country-compact'])
            replace_yesno(page2.select('#health-expenditure'), 87, -225, ind['health-expenditure'])
            replace_yesno(page2.select('#health-per-capita'), 87, -245, ind['health-per-capita'])
            replace_yesno(page2.select('#rmnch'), 87, -265, ind['rmnch'])
            replace_yesno(page2.select('#annual-rmnch'), 87, -285, ind['annual-rmnch'])
